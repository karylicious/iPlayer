...
Public Class JanelaPrincipal
    ...
    Dim DataActual, DTAdicao As Date
    ...
    Dim nao As Boolean = False
    Dim ItemExistente As Boolean = False
    ...
    Dim nAno, TaxaBits, NumMusica, NumHistorico, minutos, Seg, horas, Meses, Anos, Semanas, dias, PesqPor, n1, contador1, contador2, n2, n3, contador3, _
    DiaActual, MesActual, LTamanhoFinalMusica, RTamanhoFinalMusica, TamanhoInicioMusica, NumFormato, _
     SegundosMenos, MinutosMenos, naux, pMusica, Min, numTag, numLenMusica, numLenInterprete, _
    LTamanhoFinalInterprete, RTamanhoFinalInterprete, indexAReprod, pInterprete, TamanhoInicioInterprete, _
    TamanhoInicioAlbum, LTamanhoFinalAlbum, IndexMin, y, PlaylistActual, Musica, indexReprodRecent, CountReprodRecen, CountAcresRecen, indexAcresRecen, NumMusicaAReprod, k, numLenAlbum, pAlbum, RTamanhoFinalAlbum As Integer
    Dim KB, MB As Double
    Dim Nreprodução As Boolean = False
    Dim TemCapaBool As Boolean = False
    Dim perdeuFocus As Boolean = False
    Dim CapaSemBool As Boolean = False
    Dim FimPesq As Boolean = False
    Dim loading = True
    Dim valido As Boolean = False
    Dim ClickBtn As Boolean = False
    Dim btn As Button
    Dim DataTablePlaylist, DataTableHistorico As DataTable
    Public DataTableSemNome As DataTable
    ...
    Private Sub Form1_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If playlist.count >= 1 Then
            If verificaCdDados = False Then
                verificaCD = True
                BtnAudioCD.Text = playlist.Item(0).getItemInfo("Album")
            Else
                verificaCD = False
            End If
        Else
    ...
    Public Sub PreencherPlaylist()
        Dim Duracao, duracaoT As String
        If FimPesq = False Then
            If PlaylistActual <> 6 And PlaylistActual <> 3 Then
                Try
        ...
        For i As Integer = 0 To DataTableSemNome.Rows.Count - 1
            If FimPesq = False Then
                If PlaylistActual = 6 OrElse PlaylistActual = 3 Then
                    i = indexReprodRecent
                    p = CountReprodRecen
                Else
                    p = i + 1
            ...
            If FimPesq = False Then
                If PlaylistActual = 6 Then
                    Musica = DataTableSemNome.Rows(i).Item("NumMusica")
                    iPlayer_DAL.Playlist.CriarRelacao(6, Musica)
                ElseIf PlaylistActual = 3 Then
                    iPlayer_DAL.Playlist.CriarRelacao(3, Musica)
                End If
            End If
            ...
            Else
                If FimPesq = False Then
                    Try
                        If verificaReproduzir = False Then
                            MusicasATocar(p - 1) = DataTableSemNome.Rows(i).Item("NumMusica")
                            NomesAMostrar(p - 1) = DataTableSemNome.Rows(i).Item("NomeMusica")
                            AlBunsAMostrar(p - 1) = DataTableSemNome.Rows(i).Item("NomeAlbum")
                            InterAMostrar(p - 1) = DataTableSemNome.Rows(i).Item("NomeInterprete")
                            DuracaoAMostrar(p - 1) = DataTableSemNome.Rows(i).Item("Duracao")
                        End If
                    Catch ex As Exception
                    End Try
                Else
            ...
            End If
            num = MB
            If PlaylistActual = 5 Then
                If i = 25 Then
                    Exit For
                End If
            ElseIf PlaylistActual = 6 OrElse PlaylistActual = 3 Then
                If FimPesq = False Then
                    Exit For
                End If
            End If
    ...
    Private Sub TextBoxPesq_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TextBoxPesq.TextChanged
        If verificaPesquisa = True Then
            ...
            If TextBoxPesq.Text = "" Or TextBoxPesq.Text = "Pesquisar" Then
                ...
                DataTableSemNome = iPlayer_DAL.Playlist.MostrarUsandoNumPlaylist(PlaylistActual)
                FimPesq = True
                PreencherPlaylist()
                Exit Sub
    ...
    Sub PreencherCDAudio()
        ...
        If PlaylistActual = naux Then
            ...
            End If
        End If
        'DataTableSemNome = iPlayer_DAL.Playlist.MostrarUsandoNumPlaylist(1) nao sei se é pra q 
    End Sub
        ...
        If verificaPesquisa = False Then
            Try
                If verificaReproduzir = True Then
                    ...
                    If verificaRepeatOne = True Then 'shuffle desligado 
        ...
        If verificaPesquisa = False Then
            Try
                If verificaReproduzir = True Then
                    ...
                    If verificaRepeatOne = True Then '  shuffle ligado
                        If verificaShuffle = True Then
                            If ClickBtn = True Then
                                CountAReprod -= 1
                                indexAReprod = r.Next(0, NumReproduzidas.Count)
    ...
    Private Sub JanelaPrincipal_FormClosing(ByVal sender As System.Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles MyBase.FormClosing
        iPlayer_DAL.Playlist.Remover2(6)
        iPlayer_DAL.Playlist.Remover2(3)
        If b = False Then
            TimerOpacity.Start()
            e.Cancel = True
            b = True
        ...
        If b.Tag <> PlaylistActual Then
            ...
            If verificaReproduzir = True Then '3
                ...
                If b.Tag = naux Then '5
                    PararToolStripMenuItem15.Visible = True
                    ...
                    End If
                Else '5
                    PararToolStripMenuItem15.Visible = False
            ...
            Select Case PlaylistActual
                ...
                Case 3 'AcresRecentemente
                    ...
                    DataTableSemNome = iPlayer_DAL.Playlist.MostrarUsandoNumPlaylist(1)
                    If DataTableSemNome.Rows.Count > 0 Then
                        For p As Integer = 0 To DataTableSemNome.Rows.Count - 1
                            DTAdicao = DataTableSemNome.Rows(p).Item("DataAdicao")
                            If DTAdicao.Month = Now.Month Then
                                If DTAdicao.Day - Now.Day <= 10 Then

                                    If verificaReproduzir = False Then
                                        ReDim Preserve MusicasATocar(CountReprodRecen)
                                        ReDim Preserve NomesAMostrar(CountReprodRecen)
                                        ReDim Preserve AlBunsAMostrar(CountReprodRecen)
                                        ReDim Preserve InterAMostrar(CountReprodRecen)
                                        ReDim Preserve DuracaoAMostrar(CountReprodRecen)
                                    End If
                                    CountReprodRecen += 1
                                    indexReprodRecent = p
                                    PreencherPlaylist()
                                End If
                            ElseIf Now.Month - DTAdicao.Month = 1 Then
                                Dim DiaDoAno = Now.Day + 31
                                If DiaDoAno - DTAdicao.Day <= 10 Then
                                    CountReprodRecen += 1
                                    indexReprodRecent = p
                                    PreencherPlaylist()
                                End If
                ...
                Case 4 'Anos 2000-2010
                    DataTableSemNome = iPlayer_DAL.Playlist.MostrarUsandoNumPlaylist(4)
                    PreencherPlaylist()
                Case 5 'As 25 mais reprod
                    DataTableSemNome = iPlayer_DAL.Playlist.Mostrar25maisReprod
                    PreencherPlaylist()
                Case 6 'ReprodRecentemente
                    ListView1.Items.Clear()
                    If verificaReproduzir = False Then
                        ReDim MusicasATocar(DataTableSemNome.Rows.Count - 1)
                        ReDim NomesAMostrar(DataTableSemNome.Rows.Count - 1)
                    ...
                    If DataTableSemNome.Rows.Count > 0 Then
                        For p As Integer = 0 To DataTableSemNome.Rows.Count - 1
                            Try
                                DTAdicao = DataTableSemNome.Rows(p).Item("UltimaReproducao")
                                If DTAdicao.Month = Now.Month Then
                                    If DTAdicao.Day - Now.Day <= 10 Then
                                        If verificaReproduzir = False Then
                                            ReDim Preserve MusicasATocar(CountReprodRecen)
        ...
        Select Case tagcontextmenustrip
            ...
            Case Else
                ...
                If indexes.Count > 1 Then
                    ReDim ArrayNum(indexes.Count - 1)
                    Dim n As Integer
                    Dim ArrayNome(indexes.Count - 1) As String
                    For i As Integer = 0 To indexes.Count - 1
                        For Each n In indexes
                            ArrayNum(i) = ListView1.Items(n).SubItems(10).Text
                            ArrayNome(i) = ListView1.Items(n).SubItems(0).Text
                            i += 1
                        Next
                        Exit For
                    Next

                    For i As Integer = 0 To ArrayNum.Count - 1
                        If DataTablePlaylist.Rows.Count = 0 Then
                            iPlayer_DAL.Playlist.CriarRelacao(Tag, ArrayNum(i))
                        Else
                            For h As Integer = 0 To DataTablePlaylist.Rows.Count - 1
                                If ArrayNum(i) = DataTablePlaylist.Rows(h).Item("NumMusica") Then
                                    Dim l As String = ArrayNome(i)
                                    System.Windows.Forms.MessageBox.Show("Não foi possível adicionar a música" & " " & "'" & l.Substring(5, l.Length - 5) & "'," & " " & "visto que esta já existe na lista de reprodução indicada.", "iPlayer", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                                    ItemExistente = True
                                    Exit For
                                Else
                                    ItemExistente = False
        ...
        Select Case m
            ...
            Case 0
                ...
                Else
                    ...
                    ListView1.Items(IndexSelec).SubItems(9).Text = NumStar
                End If
    ...
    Sub parar()
        ...
        PararToolStripMenuItem15.Enabled = False
        ReproduzirToolStripMenuItem1.Text = "Reproduzir"
        ReproduzirToolStripMenuItem.Text = "Reproduzir"
        AnteriorToolStripMenuItem.Enabled = False
        AnteriorToolStripMenuItem1.Enabled = False
    ...
    Private Sub AxWindowsMediaPlayer1_CdromMediaChange(ByVal sender As Object, ByVal e As AxWMPLib._WMPOCXEvents_CdromMediaChangeEvent) Handles AxWindowsMediaPlayer1.CdromMediaChange
        ...
        Else
            EjectarDisco()
            BtnMusicas.BackColor = Color.LightBlue
            BtnMusicas.FlatAppearance.MouseDownBackColor = Color.LightBlue
            BtnMusicas.FlatAppearance.MouseOverBackColor = Color.LightBlue
        End If
    End Sub
    ...
    Sub CoisasDoEliminar()
        ...
        If ListView1.Items.Count = indexes.Count Then
            ...
            LabelTudo.Text = ""
            BtnPlay.Image = iPlayer.My.Resources.playerpp_enabled
            BtnAnterior.Image = iPlayer.My.Resources.player_rew_enabled
            BtnSeguinte.Image = iPlayer.My.Resources.player_fwd_32_enabled
        Else
            DataTableSemNome = iPlayer_DAL.Playlist.MostrarUsandoNumPlaylist(PlaylistActual)
            PreencherPlaylist()
    ...
    Sub EliminarMusica()
        ...
        ElseIf ListView1.SelectedItems.Count > 1 Then
            Dim respostas As DialogResult = System.Windows.Forms.MessageBox.Show("Tem a certeza que deseja eliminar as músicas seleccionadas ?", "iPlayer", MessageBoxButtons.YesNo, MessageBoxIcon.Question)
    ...
    Sub verificaUrl()
        ...
        str = ListView1.Items(indexAReprod).SubItems(0).Text
        ...
        If resposta = DialogResult.Yes Then
            ...
            If OpenFileDialog1.ShowDialog = DialogResult.OK Then
                ...
                iPlayer_DAL.Musica.Alterar(Nothing, Nothing, Nothing, Nothing, NumMusica, NovoCaminho)
                caminho = NovoCaminho
            End If
        Else
            verificaReproduzir = False
            ...
            If RadioOn = False Then
                If verificAddFile = False Then
                    ...
                    If naux = PlaylistActual Then
                        ...
                        PararToolStripMenuItem15.Enabled = True
                    Else
                        Reproduzir()
                    End If
    ...
    Private Sub ColarToolStripMenuItem2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ColarToolStripMenuItem2.Click
                ...
                If ListView1.Items.Count <> 0 Then
                    For j As Integer = 0 To ListView1.Items.Count - 1
                        If CopyArray(i) = ListView1.Items(j).SubItems(10).Text Then
                            ...
                            System.Windows.Forms.MessageBox.Show("Não foi possível adicionar a música" & " " & "'" & n.Substring(3, n.Length - 3) & "'," & " " & "visto que esta já existe na lista de reprodução actual.", "iPlayer", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                            ItemExistente = True
                            Exit For
                        Else
                            ItemExistente = False
        ...
        ElseIf CopyArray.Count = 1 Then
            If ListView1.Items.Count <> 0 Then

                For j As Integer = 0 To ListView1.Items.Count - 1
                    If CopyArray(0) = ListView1.Items(j).SubItems(10).Text Then
                        Dim n As String = ListView1.Items(j).SubItems(0).Text
                        System.Windows.Forms.MessageBox.Show("Não foi possível adicionar a música" & " " & "'" & n.Substring(5, n.Length - 5) & "'," & " " & "visto que esta já existe na lista de reprodução actual.", "iPlayer", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                        ItemExistente = True
                        Exit For
                    Else
                        ItemExistente = False
                ...
                Next
                If ItemExistente = False Then
                    iPlayer_DAL.Playlist.CriarRelacao(PlaylistActual, CopyArray(0))
                End If
            Else
                iPlayer_DAL.Playlist.CriarRelacao(PlaylistActual, CopyArray(0))
            End If
    ...
    Private Sub AjudaDoBluePlayerToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles AjudaDoBluePlayerToolStripMenuItem.Click
        Ajuda.ShowDialog()
    End Sub
    ...
    Private Sub PararToolStripMenuItem15_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles PararToolStripMenuItem15.Click
        AxWindowsMediaPlayer1.Ctlcontrols.stop()
        parar()
    End Sub
End Class
